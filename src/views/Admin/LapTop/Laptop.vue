<template>
    <div>
        <v-row class="mt-3">
            ...
            <h4 class="ml-2">Danh sách Sản Phẩm</h4>
            <v-spacer></v-spacer>
                        ...
                        <tbody>
                            <tr v-for="(item, index) in displayed" :key="index">
                                ...
                                <td class="text-center">
                                    <v-img height="100px" width="100px" cover :src="item.hinhAnh"></v-img>
                                </td>
                                <td class="text-center">{{ item.tenLaptop }}</td>
                                <td class="text-center">{{ item.hangId }}</td>
                                ...
                                <td class="text-center">
                                    ...
                                    <v-btn color="red" size="x-small" class="mr-3" icon
                                        @click="dialogDelete = true, DeleteId = item.latopId">
        ...
        <LaptopAdd :dialogAdd="dialogAdd" @close="dialogAdd = false" @updateData="danhsach" />
        <LaptopEdit :dialogEdit="dialogEdit" @close="dialogEdit = false" @updateData="danhsach" :currentItem="currentItem" />
...
import LaptopAdd from '@/views/Admin/LapTop/LaptopAdd.vue'
import LaptopEdit from '@/views/Admin/LapTop/LatopEdit.vue'
export default {
    name: 'SachView',
    components: { LaptopAdd,LaptopEdit },
    data() {
        return {
            datas: [],
            dialogAdd: false,
    ...
    methods: {
        ...
        deleteItem() {
            axios.delete('https://localhost:7011/api/LapTop/' + this.DeleteId).then(res => {
                alert(res.data);
